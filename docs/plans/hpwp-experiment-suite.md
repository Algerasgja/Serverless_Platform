...
## 3. 脚本入口
- 对比：`analysis/experiments/compare_experiments.py`
- 消融：`analysis/experiments/ablation_experiments.py`
- 超参数：`analysis/experiments/hparam_experiments.py`

## 4. 默认输出目录
所有实验脚本按实验类型输出到 `results/` 下的分类目录（覆盖同名文件）：
- 对比：
  - 统一目录：`results/compare/metrics/*` 与 `results/compare/figures/*`
  - 汇总快照：`results/compare/compare_metrics.csv`
- 消融：`results/ablation/latest/ablation_metrics.csv`、`results/ablation/latest/ablation_pairwise.csv`、`results/ablation/latest/ablation_e2e.png`、`results/ablation/latest/ablation_gain.png`
- 超参数：`results/hparam/latest/hparam_metrics.csv`、`results/hparam/latest/hparam_tradeoff.png`、`results/hparam/latest/hparam_best.yaml`
